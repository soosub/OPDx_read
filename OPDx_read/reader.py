    ...
    def read_item(self, f):
        ...
        if item.data_type == DektakLoad.data_types["DEKTAK_BOOLEAN"]:
            ...
            try:
                ...
                f.seek(ini)
                f.read(8)
                item.data["data"] = np.frombuffer(f.read(self.current_count * 8), dtype=float)
            except UnicodeDecodeError:
                f.seek(ini)
                f.read(10)
                item.data["data"] = np.frombuffer(f.read(self.current_count * 8), dtype=float)
            ...
            data = f.read(4 * N)
            item.data["data"] = np.reshape(
                np.frombuffer(data, dtype="float32"), (item.data["yres"], item.data["xres"])
            )
    ...
    def get_data_2D(self, plot=False):
        ...
        if subitem is not None:
            ...
            if plot:
                ...
                plt.imshow(
                    mat.data["data"] * scale.data["value"],
                    extent=[0, dim2.data["value"], 0, dim1.data["value"]],
                )
                plt.xlabel(symbol_x)
                plt.ylabel(symbol_y)
            zs = mat.data["data"] * scale.data["value"] / divisor
            xs = np.linspace(0, dim1.data["value"], np.shape(mat.data["data"])[1])
            ys = np.linspace(0, dim2.data["value"], np.shape(mat.data["data"])[0])
            return (xs, ys, zs)
        else:
